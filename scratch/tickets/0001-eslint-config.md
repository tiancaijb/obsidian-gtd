...
## 任务清单
- [x] 检查 `eslint.config.mjs` 和 `eslint.config.mts` 内容，确定哪个是生效的
- [x] 合并或清理配置文件，只保留一个
- [x] 启用 `typescript-eslint` strict 规则
- [x] 添加 `eslint-plugin-vitest`（如果可用）
- [x] 确认 `npm run lint` 零错误
...
| `npm run build` | ✅ 编译通过 |
| `npm test` | ✅ 12 个测试文件、583 个测试全部通过 |
| 配置文件 | `eslint.config.mjs`（flat config） |
| 第二个配置 `.mts` | 不存在（已清理） |
| Strict 规则 | `@typescript-eslint/strictTypeChecked` 已启用 |
| vitest 插件 | `@vitest/eslint-plugin` 已配置
